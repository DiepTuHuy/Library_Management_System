"""
Script to delete old test member accounts and create new ones with proper password hashing
"""
...
def delete_and_recreate_test_members():
    """Delete old test members and create new ones with proper hashing"""
    ...
    print("\n" + "="*80)
    print("DELETING OLD TEST MEMBERS AND RECREATING WITH PROPER PASSWORD HASHING")
    print("="*80)
    
    # Delete old test members
    old_emails = ["member.ontime@library.test", "member.late@library.test"]
    for email in old_emails:
        result = users_collection.delete_one({"email": email})
        if result.deleted_count > 0:
            print(f"❌ Deleted old account: {email}")
        else:
            print(f"✓ No old account found for: {email}")
    
    # Delete old borrow records associated with these accounts (if any IDs are available)
    borrows_collection.delete_many({"user_id": {"$nin": []}})  # Just to clear old test data
    
    # ===== CREATE MEMBER 1: Returns book on time =====
    member1_data = {
    ...
    print(f"\n✅ Created Member 1: {member1_data['name']} (ID: {member1_id})")
    print(f"   Email: {member1_data['email']}")
    print(f"   Password: member123")
    
    # ===== CREATE MEMBER 2: Returns book late =====
    member2_data = {
    ...
    print(f"\n✅ Created Member 2: {member2_data['name']} (ID: {member2_id})")
    ...
    print("\n🔐 Note: Passwords are now properly hashed using PBKDF2")
    print("   You can now login with these credentials!\n")
    return True

if __name__ == "__main__":
    try:
        success = delete_and_recreate_test_members()
        if success:
            print("✅ All test members recreated successfully!")
        else:
            print("❌ Failed to recreate test members")
